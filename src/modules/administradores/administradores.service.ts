...
@Injectable()
export class AdministradoresService implements OnApplicationBootstrap {
  constructor(
  ...
  async cambiarContraseña(id: number, dto: UpdateAdministradorDto) {
    ...
    const actualPasswordComparition = await bcrypt.compare(
      dto.passwordActual,
    ...
    if (dto.passwordNueva !== dto.passwordConfirm) {
      throw new BadRequestException(
        'La nueva contraseña no coincide con el campo de confirmacion',
      );
    }

    const newPasswordHash = await bcrypt.hash(dto.passwordNueva, 10);

    return await this.administradorRepository.update(id, {
  ...
  async onApplicationBootstrap() {
    const emailPorDefecto = 'admin@misierra.com'; // O el mail de tu cliente

    // Verificamos si ya existe el usuario (Idempotencia)
    const adminExistente = await this.administradorRepository.findOne({
      where: { mail: emailPorDefecto },
    });

    if (!adminExistente) {
      console.log(
        '🌱 No se encontró administrador. Creando usuario inicial...',
      );

      const hashedPassword = await bcrypt.hash('Admin1234*', 10); // Usa una contraseña provisoria fuerte

      const nuevoAdmin = this.administradorRepository.create({
        nombre: 'Administrador',
        mail: emailPorDefecto,
      ...
      console.log('📋 El administrador inicial ya existe. Saltando seeding.');
    }
  }
}
